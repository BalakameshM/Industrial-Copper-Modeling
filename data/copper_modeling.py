...
def predict_status(ctry,itmtp,aplcn,wth,prdrf,qtlg,cstlg,tknslg,slgplg,itmdt,itmmn,itmyr,deldtdy,deldtmn,deldtyr):
    ...
    with open(r"C:\Users\Asus\Classification_model.pkl","rb") as f:
...
def predict_selling_price(ctry,sts,itmtp,aplcn,wth,prdrf,qtlg,cstlg,
    ...
    dydy= int(deldtyr)
    #modelfile of the classification
    with open(r"C:\Users\Asus\Regression_Model.pkl","rb") as f:
...
with st.sidebar:
    option = option_menu('Select Option', options=["HOME", "PREDICT SELLING PRICE", "PREDICT STATUS"])
if option == "HOME":
    st.title("**INDUSTRIAL COPPER MODELING**")
    st.image("https://samcotech.com/wp-content/uploads/2023/08/Whats-the-Best-Way-to-Remove-Copper-from-Industrial-Wastewater-2-1024x581-1.jpg")
    st.subheader("Overview:")
    st.write("This project aims to address the challenges faced by the copper industry in accurately predicting sales prices and classifying leads. In the copper industry, data related to sales and pricing often suffers from skewness and noise, which can affect the accuracy of manual predictions. Manual handling of these issues can be time-consuming and may not result in optimal pricing decisions.To tackle these challenges, this project employs machine learning techniques to preprocess the data and build predictive models")
